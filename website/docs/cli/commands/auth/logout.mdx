...
<CastPlayer title="atmos auth logout --help" src="/casts/screengrabs/atmos-auth-logout--help.cast" static chrome />
...
:::note Ambient Providers
Providers that resolve credentials from the ambient environment on every login — `gcp/adc` and
`gcp/workload-identity-federation` — have nothing to preserve in the keychain, so logout always
clears their entries whether or not you pass `--keychain`. Nothing is lost: the next
`atmos auth login` re-resolves from `gcloud auth application-default login`, `GOOGLE_APPLICATION_CREDENTIALS`,
the metadata server, or the OIDC token source.
:::
...
**Next login** (`atmos auth login --identity dev-admin`):
